function exportKnapsackResults(n, limit, αs, results)
    ...
    if n ≤ limit
        ...
        drawAndSavePlot("kn-plot-ratios-adv_rec_sel-$n-$suffix.pdf", αs, results[2:4, 1, :].', "α", "average ratio ρₖ", ["\$\rho_{Adv}\$" "\$\rho_h\$" "\$\rho_{Sel}\$"])
        df = DataFrame(a = αs, ra = results[2, 2, :], rh = results[3, 2, :], rs = results[4, 2, :], ra = results[5, 2, :])
        saveCsv("kn-plot-times-adv_rec_sel-$n-$suffix.csv", df, ["α", "ρ_Adv",  "ρ_h",  "ρ_Sel" "Eval."])
        drawAndSavePlot("kn-plot-times-adv_rec_sel-$n-$suffix.pdf", αs, results[2:5, 2, :].', "α", "average time (s)", ["\$\rho_{Adv}\$" "\$\rho_h\$" "\$\rho_{Sel}\$" "Eval."])
    end
end
...
function exportAssignmentResults(m, limit, αs, results)
    ...
    if m ≤ limit
        ...
        drawAndSavePlot("as-plot-ratios-adv_rec_sel-$m-$suffix.pdf", αs, results[2:5, 1, :].', "α", "average ratio ρₖ", ["\$\rho_{Adv}\$" "\$\rho_h\$" "\$\rho_{Sel}\$" "\$\rho_{Lag}\$"])

        df = DataFrame(a = αs, ra = results[2, 2, :], rh = results[3, 2, :], rs = results[4, 2, :], rl = results[5, 2, :], ra = results[6, 2, :])
        saveCsv("as-plot-times-adv_rec_sel-$m-$suffix.csv", df, ["α", "ρ_Adv",  "ρ_h",  "ρ_Sel", "ρ_Lag" "Eval"])
        drawAndSavePlot("as-plot-times-adv_rec_sel-$m-$suffix.pdf", αs, results[2:6, 2, :].', "α", "average time (s)", ["\$\rho_{Adv}\$" "\$\rho_h\$" "\$\rho_{Sel}\$" "\$\rho_{Lag}\$" "Eval."])
    end
end
...
function drawAndSavePlot(filename, x, ys, xlabel, ylabel, yslabels; linewidth=2, linestyles = [:solid :dash :dashdot :dot], shape = [:diamond :pentagon :star4 :utriangle], palette=cgrad([:black, :grey]), annotations = [])
    p = plot(x, ys, linewidth=linewidth, xlabel = xlabel, ylabel = ylabel, lab = yslabels, linestyle = linestyles, shape = shape, palette = palette, annotations = annotations)
    savefig(p, filename)
end
