function exportKnapsackResults(n, limit, αs, results)
    ...
    if n ≤ limit
        ...
        df = DataFrame(a = αs, ra = results[2, 2, :], rh = results[3, 2, :], rs = results[4, 2, :], re = results[5, 2, :])
        saveCsv("kn-plot-times-adv_rec_sel-$n-$suffix.csv", df, ["α", "ρ_Adv",  "ρ_h",  "ρ_Sel", "Eval."])
        drawAndSavePlot("kn-plot-times-adv_rec_sel-$n-$suffix.pdf", αs, results[2:5, 2, :].', "α", "average time (s)", [L"\rho_{Adv}" L"\rho_h" L"\rho_{Sel}" "Eval."])
    end
end
...
function exportAssignmentResults(m, limit, αs, results)
    ...
    if m ≤ limit
        ...
        df = DataFrame(a = αs, ra = results[2, 2, :], rh = results[3, 2, :], rs = results[4, 2, :], rl = results[5, 2, :], re = results[6, 2, :])
        saveCsv("as-plot-times-adv_rec_sel-$m-$suffix.csv", df, ["α", "ρ_Adv",  "ρ_h",  "ρ_Sel", "ρ_Lag", "Eval."])
        drawAndSavePlot("as-plot-times-adv_rec_sel-$m-$suffix.pdf", αs, results[2:6, 2, :].', "α", "average time (s)", [L"\rho_{Adv}" L"\rho_h" L"\rho_{Sel}" L"\rho_{Lag}" "Eval."])
    end
end
