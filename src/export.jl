function exportKnapsackResults(n, limit, αs, results)
    ...
    drawAndSavePlot("kn-plot-ratios-rec-$n-$suffix.pdf", αs, results[1, 1, :], L"α", L"average\,ratio\,\rho(c_0)", "n = $n")

    df = DataFrame(a = αs, b = results[1, 2, :])
    saveCsv("kn-plot-times-rec-$n-$suffix.csv", df, ["α", "ρ(c_0)"])
    drawAndSavePlot("kn-plot-times-rec-$n-$suffix.pdf", αs, results[1, 2, :], L"α", L"average\,time\,(s.)", "n = $n")
    ...
    if n ≤ limit
        ...
        drawAndSavePlot("kn-plot-ratios-adv_rec_sel-$n-$suffix.pdf", αs, results[2:4, 1, :].', L"α", L"average\,ratio\,\rho_k", [L"\rho_{Adv}" L"\rho_h" L"\rho_{Sel}"])
        ...
        drawAndSavePlot("kn-plot-times-adv_rec_sel-$n-$suffix.pdf", αs, results[2:4, 2, :].', L"α", L"average\,time\,(s.)", [L"\rho_{Adv}" L"\rho_h" L"\rho_{Sel}"])
    end
end
...
function exportAssignmentResults(m, limit, αs, results)
    ...
    drawAndSavePlot("as-plot-ratios-rec-$m-$suffix.pdf", αs, results[1, 1, :], L"α", L"average\,ratio\,\rho(c_0)", "m = $m")

    df = DataFrame(a = αs, b = results[1, 2, :])
    saveCsv("as-plot-times-rec-$m-$suffix.csv", df, ["α", "ρ(c_0)"])
    drawAndSavePlot("as-plot-times-rec-$m-$suffix.pdf", αs, results[1, 2, :], L"α", L"average\,time\,(s.)", "m = $m")
    ...
    if m ≤ limit
        ...
        drawAndSavePlot("as-plot-ratios-adv_rec_sel-$m-$suffix.pdf", αs, results[2:5, 1, :].', L"α", L"average\,ratio\,\rho_k", [L"\rho_{Adv}" L"\rho_h" L"\rho_{Sel}" L"\rho_{Lag}"])
        ...
        drawAndSavePlot("as-plot-times-adv_rec_sel-$m-$suffix.pdf", αs, results[2:5, 2, :].', L"α", L"average\,time\,(s.)", [L"\rho_{Adv}" L"\rho_h" L"\rho_{Sel}" L"\rho_{Lag}"])
    end
end
